 ...
 */
export function createSilentAudioData(length = 1024, sampleRate = 48000): MockAudioData {
  return {
 ...
 */
export function createFullScaleAudioData(length = 1024, sampleRate = 48000): MockAudioData {
  return {
 ...
 */
export function createFrequencyAudioData(
  distribution: 'flat' | 'bass' | 'treble' | 'mid',
  length = 1024,
  sampleRate = 48000
): MockAudioData {
 ...
 */
export function expectUnique(array: unknown[]): void {
  const seen = new Set();
 ...
 */
export async function expectExecutionTimeUnder<T>(
  fn: () => T | Promise<T>,
  maxDuration: number
): Promise<T> {
  ...
  if (duration > maxDuration) {
    throw new Error(
      `Function took ${duration.toFixed(2)}ms, expected under ${maxDuration}ms`
    );
  }
  return result;
}
 ...
 */
export function createDelayedError(
  message: string,
  delay = 100
): Promise<never> {
